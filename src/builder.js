...
export async function buildPortfolioSite(paths) {
  ...
  try {
    ...
    for (const route of config.routes) { const sourceId = route.presentation.modelSourceId; const model = resolved.get(sourceId)?.value; if (!model) throw new BuilderError("route.invalid", "Presentation source missing", { routePath: route.path, sourceId }); const Renderer = route.presentation.kind === "cv" ? CV : Portfolio; const html = `<!doctype html><html lang="${config.metadata.language ?? "en"}"><head><meta charset="utf-8"><title>${route.metadata?.title ?? config.metadata.title}</title></head><body>${renderToStaticMarkup(React.createElement(Renderer, { model }))}<script type="application/json" id="szd-portfolio-bootstrap">${JSON.stringify({ version: 1, routePath: route.path, mode: "build-only", modelVersions: [{ sourceId, version: 1 }], buildModels: [{ sourceId, value: model }], browserSourceIds: [] }).replaceAll("<", "\\u003c")}</script><script type="module" src="/assets/szd-portfolio-bootstrap.js"></script></body></html>`; await write(join(staging, outputFile(route.path)), html); }
