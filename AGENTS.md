...
This file is binding for every agent session in this repository.
...
route unless its caller explicitly supplies it. Missing optional navigation
...
attributes, CSS tokens, schemas, error codes, export names, and export paths are
semver-governed. Every exported validator has positive and negative tests, and
every rejection branch is exercised.

Never publish an npm version without explicit authorization.

## Source of truth

In precedence order:

1. `design/00-brief.md` — problem, scope, non-goals, definition of done
2. `design/20-contract.md` — public surface, invariants, errors, compatibility
3. `design/10-design.md` — ownership, architecture, inventory, control flow
4. `design/30-slices.md` — phased implementation and acceptance criteria
5. `design/90-decisions.md` — append-only decisions and rejected alternatives

If code and contract disagree about meaning, stop and identify the conflict.
Do not silently reconcile it. Correct descriptive drift only after verifying
the tree completely.
...
- Consumer-owned data types and validators remain outside this package. A
  consumer adapter projects validated raw values into package view models.
- Package validators accept `unknown`. A cast never substitutes for validation.
- Resolution completes, raw consumer validation succeeds, projection runs, and
  package view-model validation succeeds before a renderer receives data.
- Multi-source failures are collected in declaration order. Composition never
  receives a partial source set.
- Fallback is explicit. A fallback result still carries the triggering error;
...
A rule has one home. Documents state invariants, ownership, reasons, and failure
semantics; code owns declarations, exact field lists, and signatures. When a
document must repeat a public shape to remain self-contained, it names the
future declaration as canonical and changes with it in the same release.
...
Verification for a release includes typecheck, unit tests, positive and
negative validator tests, SSR rendering, accessibility checks, CSS namespace
checks, DOM contract fixtures, optional-entry dependency-isolation checks, a
packed-tarball consumer, and Docusaurus compatibility fixtures. Report every
...
Record choices in `design/90-decisions.md` as:
